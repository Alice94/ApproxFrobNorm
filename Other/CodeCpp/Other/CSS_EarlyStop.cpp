...
long int SelectNextColumn_EarlyStop(long int* m, long int* n, double* B, long int* k, long int* t, double* upperBound, double* colNum);
...
long int SelectNextColumn_EarlyStop(long int* m, long int* n, double* B, long int* k, long int* t, double* upperBound, double* colNum) {
  *colNum = 0;
  ...
  // Do the SVD of B, result is saved in s and U
  char jobu = 'S';
  char jobvt = 'N';
  ...
  // PREPROCESSING: compute the norms of all the columns of B and sort them from the biggest to the smallest
  // This should increase the chance of finding the next column more quickly
  pair<double, long int>* columns = (pair<double, long int>*)malloc((*n)*(sizeof(long int) + sizeof(double)));
  for (long int i=0; i<*n; i++) {
    columns[i].first = dnrm2_(m, B+i*(*m), &one);
    columns[i].second = i;
  }
  ...
  for (long int ii=(*n)-1; ii>=0; ii--) {
    ...
    *colNum = *colNum + 1;
    ...
    if (MinRatio < 0 || ratio*(*k-*t)<=*upperBound) {
      MinRatio = ratio;
      bestCol = i;
      if (ratio*(*k-*t)<=*upperBound) {
	satisfied = true;
	break; // it means we are satisfied with the current choice.
      }
    }
  }
